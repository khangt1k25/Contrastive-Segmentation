...
def train(p, train_loader, model, optimizer, epoch, amp):
    ...
    recon_losses = AverageMeter('Reconstruction', ':.4e')

    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(len(train_loader), 
                        [losses, contrastive_losses, superpixel_losses, recon_losses, saliency_losses, top1, top5],
    ...
    for i, batch in enumerate(train_loader):
        ...
        logits, labels, obj_logits, obj_labels, saliency_loss, recon_loss = model(im_q=im_q, im_k=im_k, sal_q=sal_q, sal_k=sal_k)
        ...
        loss = contrastive_loss + saliency_loss + superpixel_loss + recon_loss
    ...
    writer.add_scalar('reconstruction loss', recon_losses.avg, epoch)
    writer.close()      
