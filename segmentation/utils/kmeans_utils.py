...
def save_embeddings_to_disk(p, val_loader, model, n_clusters=21, seed=1234):
    ...
    all_prototypes = torch.zeros((len(val_loader.sampler) * reducer, 32)).cuda()
    ...
    for i, batch in enumerate(val_loader):
        ...
        output = torch.reshape(output, [-1, dim]) # queries: BHW x dim
    ...
    centroids = torch.tensor(kmeans.cluster_centers_, requires_grad=False).cuda() # C x dim
    ...
    # save predictions
    for i, fname, pred in zip(range(len(val_loader.sampler)), names, predicted):
        np.save(os.path.join(p['embedding_dir'], fname + '.npy'), predicted)
        if i % 300 == 0:
