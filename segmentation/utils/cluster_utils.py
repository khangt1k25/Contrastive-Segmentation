...
def eval_cluster(p, val_dataset, n_clusters=21, compute_metrics=False, verbose=True):
    ...
    for i, sample in enumerate(val_dataset):
        ...
        # Load embedding
        filename = os.path.join(p['cluster_dir'], sample['meta']['image'] + '.npy')
...
def save_embeddings_to_disk(p, val_loader, model, n_clusters=21, seed=1234):
    ...
    all_prototypes = torch.zeros((len(val_loader.sampler), n_clusters-1)).cuda()
    all_sals = torch.zeros((len(val_loader.sampler), 512, 512)).cuda()
    names = []
    for i, batch in enumerate(val_loader):
        _, sal, output = model(batch['image'].cuda(non_blocking=True))
        meta = batch['meta']
    ...
    prediction_clusters = np.argmax(all_prototypes, axis=1)

    # save predictions
    for i, fname, pred in zip(range(len(val_loader.sampler)), names, prediction_clusters):
        prediction = all_sals[i].copy()
        prediction[prediction == 1] = pred + 1
        np.save(os.path.join(p['cluster_dir'], fname + '.npy'), prediction)
        if i % 300 == 0:
